...
export default function App() {
  ...
  const { mode, setMode, updateGrid, recordEpisode, resetStats, incrementStep } = useDemoStore();

  const syncGrid = useCallback(() => {
    const g = gridRef.current;
    updateGrid({ agentRow: g.agentRow, agentCol: g.agentCol, targetRow: g.targetRow, targetCol: g.targetCol, trail: [...g.trail], gridSize: g.gridSize });
  }, [updateGrid]);
  ...
  const createEnv = useCallback(() => {
    ...
    if (!origStepRef.current) {
      ...
      grid.step = (action: number | number[]) => { origStepRef.current!(action); episodeRewardRef.current += grid.reward(); incrementStep(); syncGrid(); };
      grid.reset = () => { recordEpisode(episodeRewardRef.current); episodeRewardRef.current = 0; origResetRef.current!(); syncGrid(); };
    }
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#0a0a1a', color: '#e2e8f0', fontFamily: 'system-ui, sans-serif' }}>
      <header style={{ textAlign: 'center', padding: '24px 0 8px', position: 'relative' }}>
        ...
        <h1 style={{ margin: 0, fontSize: 24, fontWeight: 700 }}>Ignition<span style={{ color: '#6366f1' }}>AI</span> <span style={{ fontSize: 14, fontWeight: 400, color: '#888' }}>GridWorld</span></h1>
      ...
      <div style={{ display: 'flex', gap: 24, padding: '16px 32px', maxWidth: 1200, margin: '0 auto', alignItems: 'flex-start' }}>
        ...
        <div style={{ flex: '0 0 auto', border: `3px solid ${borderColor}`, borderRadius: 10, padding: 2 }}><GridCanvas /></div>
