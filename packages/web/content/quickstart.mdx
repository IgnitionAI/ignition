...
env.infer()  // Watch the trained agent play
```

`save()` stores the model weights **and** the agent's internal state (`epsilon`, step counter, best reward). `load()` restores everything. No manual bookkeeping.
