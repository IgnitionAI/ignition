...
1. **`@ignitionai/core`** provides `CartPoleEnv` (a built-in `TrainingEnv`), the `TrainingEnv` interface contract itself, and the `IgnitionEnv` base class with the generic training loop.
